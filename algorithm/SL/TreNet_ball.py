...
from base.env.ball import DoubleColorBall
...
class Algorithm(BaseSLTFModel):
    ...
    def _init_nn(self):
        ...
        # self.cnn_x_input is a [-1, 5, 20, 1] tensor, after cnn, the shape will be [-1, 5, 20, 2].
...
def main(args):
    ...
    env = DoubleColorBall(start_date="2003-01-01", end_date="2020-12-31", **{
        "use_sequence": True,
        "seq_length": 10,
        "scaler": MinMaxScaler,
    ...
    algorithm = Algorithm(tf.Session(config=config), env, env.seq_length, env.data_dim, env.data_dim, **{
        "mode": mode,
        "hidden_size": 5,
        "enable_saver": True,
        "train_steps": train_steps,
        "enable_summary_writer": True,
        "save_path": os.path.join(CHECKPOINTS_DIR, "SL", model_name, "ball", "model"),
        "summary_path": os.path.join(CHECKPOINTS_DIR, "SL", model_name, "ball", "summary"),
    })
